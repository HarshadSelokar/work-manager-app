...
export const EditWorkScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (data) {
        ...
        Alert.alert('Error', 'Task not found in records.');
        navigation.goBack();
      }
    } catch (err) {
  ...
  const handleSave = useCallback(() => {
    ...
    if (cleanRef) {
      const exists = worksRepo.referenceExists(cleanRef, workId); // Exclude current ID!
      if (exists) {
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text variant="bodyMedium" color="textSecondary">
            Modify values to update task records.
          </Text>
        </View>
        ...
        <View style={styles.actionRow}>
          <Button title="Save Changes" variant="primary" onPress={handleSave} style={styles.actionBtn} />
          <Button
            title="Cancel"
            variant="secondary"
